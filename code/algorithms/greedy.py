...
def greedy_algorithm(iterations, water_layout, max_houses):
 # standard neighbourhood distribution of the houses
    ...
    for i in range(max_houses):
        ...
        highest_score = scorecalculator(neighbourhood)
        table.append([i, max_houses, highest_score, best_house.coordinates, best_house.name])

    # save the information from the iteration
    df_greedy = pd.DataFrame(table, columns = ["iteration", "max_houses","score", "coordinates", "type"]).set_index("iteration")
